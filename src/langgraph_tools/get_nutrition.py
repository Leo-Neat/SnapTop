...
def get_fatsecret_token(client_id: str, client_secret: str) -> str:
    ...
    response = requests.post(FATSECRET_TOKEN_URL, headers=headers, data=data, timeout=10)
    if response.status_code != 200:
        raise NutritionAPIError(f"FatSecret token error {response.status_code}: {response.text}")
...
def get_nutrition(query: str) -> dict:
    ...
    response = requests.get(FATSECRET_API_URL, headers=headers, params=params, timeout=10)
    if response.status_code != 200:
        raise NutritionAPIError(f"FatSecret API error {response.status_code}: {response.text}")
    data = response.json()
